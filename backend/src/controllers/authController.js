...
function makeToken(user) {
  return jwt.sign(
    { id: user.id, email: user.email, role: user.role, ad: user.ad, soyad: user.soyad },
    SECRET,
    { expiresIn: '7d' }
  )
}
...
async function register(req, res, next) {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO users (ad, soyad, telefon, email, sifre) VALUES (?,?,?,?,?)',
    ...
    const [rows] = await db.query('SELECT id, ad, soyad, email, role FROM users WHERE id = ?', [userId])
    return res.status(201).json({ success: true, token: makeToken(rows[0]), user: rows[0] })
  } catch (err) { next(err) }
}
  ...
  try {
    ...
    if (kod && !email) {
      ...
      if (kodRow.kullanildi && kodRow.user_id) {
        ...
        const [userRows] = await db.query('SELECT id,ad,soyad,email,role FROM users WHERE id=?', [kodRow.user_id])
    ...
    if (kod && email) {
      ...
      // Kullanıcı var mı?
      let [userRows] = await db.query('SELECT id,ad,soyad,email,role FROM users WHERE email=?', [email.toLowerCase().trim()])
      let userId

      if (!userRows.length) {
        // Yeni kullanıcı oluştur
        const [ins] = await db.query(
          'INSERT INTO users (ad,soyad,email,role) VALUES (?,?,?,?)',
          ['', '', email.toLowerCase().trim(), 'user']
        )
        userId = ins.insertId
        await db.query('INSERT INTO user_abonelik (user_id, aktif) VALUES (?,0)', [userId])
        ;[userRows] = await db.query('SELECT id,ad,soyad,email,role FROM users WHERE id=?', [userId])
      } else {
...
async function me(req, res, next) {
  try {
    const [rows] = await db.query(
      `SELECT u.id, u.ad, u.soyad, u.email, u.telefon, u.role,
