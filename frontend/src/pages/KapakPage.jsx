...
export default function KapakPage() {
  ...
  const [leftPos,  setLeftPos]  = useState({ x: 4.63, y: 55 })
  ...
  function handleReset() {
    ...
    setBgZoom(1); setBgRotate(0); setBgMirror(false); setBgOffsetX(0); setBgOffsetY(0)
    setFgZoom(1); setFgRotate(0); setFgMirror(false)
    setLeftPos({ x: 4.63, y: 55 }); setRightPos({ x: 55, y: 32 })
  ...
  function startTextDrag(e, side) {
    ...
    function move(ev) {
      ...
      const ny = Math.max(22, Math.min(72 - textH, startPosY + ((cy - startY) / rect.height) * 100))
